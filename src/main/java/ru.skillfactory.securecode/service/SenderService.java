...
public class SenderService {
    ...
    public SenderService(Connection connection) {
        this.userDao = new UserDao(connection);
        logger.debug("SenderService initialized");
    }
    ...
    public void sendOtp(UUID userId, String code) {
        ...
        try {
            ...
            logger.debug("User found: email={}, phone={}, telegramId={}", user.email, user.phone, user.telegramId);

            sendEmail(user.email, code);
            smsSend(user.phone, code);
            sendTelegram(user.telegramId, code);

            logger.debug("OTP sent successfully for userId={}", userId);
